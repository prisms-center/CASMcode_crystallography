...
double fcc_lattice_parameter() { return 1.75; }
...
xtal::Lattice fcc_superlattice() {
  auto fcc_superlattice =
      xtal::make_superlattice(::fcc_lattice(), ::transformation_matrix());
...
TEST(UnitCellCoordTest, construct_from_coordinate) {
  Eigen::Vector3d exact_cart =
      fcc_lattice().lat_column_mat() * Eigen::Vector3d(2, 0, 5);
  Coordinate exact_coord(exact_cart, fcc_lattice(), CART);
  EXPECT_EQ(UnitCell::from_coordinate(exact_coord), UnitCell(2, 0, 5))
      << "Failed with exact coordinate";

  Eigen::Vector3d fuzzy_cart =
      exact_cart - Eigen::Vector3d(0.00001, -0.00002, -0.000005);
  Coordinate fuzzy_coord(fuzzy_cart, fcc_lattice(), CART);
  EXPECT_EQ(UnitCell::from_coordinate(fuzzy_coord), UnitCell(2, 0, 5))
      << "Failed with fuzzy coordinate";
...
TEST(UnitCellCoordTest, reset_tiling_unit) {
  ...
  UnitCell relative_to_stacked_uc =
      relative_to_conventional_uc.reset_tiling_unit(
          conventional_lattice, stacked_conventional_lattice);
  EXPECT_EQ(relative_to_stacked_uc, UnitCell(4, 4, 2));

  relative_to_stacked_uc = UnitCell(4, 4, 4);
  relative_to_conventional_uc = relative_to_stacked_uc.reset_tiling_unit(
      stacked_conventional_lattice, conventional_lattice);
  EXPECT_EQ(relative_to_conventional_uc, UnitCell(4, 4, 8));

  bool good_catch = false;
  relative_to_conventional_uc = UnitCell(4, 4, 3);
  try {
    relative_to_stacked_uc = relative_to_conventional_uc.reset_tiling_unit(
        conventional_lattice, stacked_conventional_lattice);
  } catch (const std::runtime_error &e) {
    good_catch = true;
  }
