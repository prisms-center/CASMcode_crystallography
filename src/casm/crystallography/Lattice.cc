...
double Lattice::angle(Index i) const {
  return (180.0 / M_PI) *
         CASM::angle(m_lat_mat.col((i + 1) % 3), m_lat_mat.col((i + 2) % 3));
}
...
std::vector<int> Lattice::calc_kpoints(std::vector<int> prim_kpoints,
                                       Lattice prim_lat) {
  ...
  double prim_density = (prim_kpoints[0] * prim_kpoints[1] * prim_kpoints[2]) /
                        (prim_recip_lat.volume());
  ...
  double shortest =
      *std::min_element(prim_vec_lengths.begin(), prim_vec_lengths.end());
  ...
  super_density = (super_kpoints[0] * super_kpoints[1] * super_kpoints[2]) /
                  (recip_lat.volume());
 ...
 *
 * First, the translation vectors of the primitive cell are determined, then
 * reduced. The Bravais lattice is determined using Niggli's transformations.
 * The method used to determine the basis vectors are to find body and face
 * diagonals across the cell. The vectors satisfying these main conditions can
 * be found by ensuring: i) the shortest face diagonal is not shorter than the
 * longest edge of the same face ii) the shortest body diagonal is not shorter
 * than the longest edge of the cell
 *
 * This is accomplished by enumerating the skew matrices (identity matrix with
 * one of the off-diagonal terms =1 or =-1).  This will replace one of the
 * lattice vectors with the face diagonal corresponding to that linear
 * combination.  If the length of any of the new skewed lattice vectors is
 * shorter than the length of the original cell, it is replaced. All skew
 * matrices have determinants equal to 1, which preserves the volume of the
 * original cell.
 *
 */
Lattice Lattice::reduced_cell2() const {
  std::vector<Eigen::Matrix3d> const &skew(skew_transforms());
  std::vector<Eigen::Matrix3d> const &ntrans(
      NiggliRep::cell_invariant_transforms());
  ...
  while (!minimized) {
    ...
    for (Index i = 0; i < skew.size(); ++i) {
      ...
      t_imetric = skew[(i + 12) % 12].transpose() * reduced_imetric *
                  skew[(i + 12) % 12];

      if (almost_equal(t_metric.trace(), reduced_metric.trace(),
                       tol() * tol())) {
        if (almost_equal(t_imetric.trace(), reduced_imetric.trace(),
                         tol() * tol())) {
          if (niggli_index(lat_column_mat() * trans * skew[i], tol()) >
              niggli_index(lat_column_mat() * trans, tol())) {
    ...
    for (Index i = 0; i < ntrans.size(); ++i) {
      Index tb_index =
          niggli_index(lat_column_mat() * trans * ntrans[i], tol());
      if (tb_index > b_index) {
        b_index = tb_index;
 ...
 * \brief This function finds the reduced cell from the given primitive cell.
 *
 * This implementation is the LLL algorithm as laid out by Hoffstein, Jeffrey;
 * Pipher, Jill; Silverman, J.H. (2008). An Introduction to Mathematical
 * Cryptography. Springer. ISBN 978-0-387-77993-5. Some corrections have been
 * made from  Silverman, Joseph. "Introduction to Mathematical Cryptography
 * Errata" (PDF). Brown University Mathematics Dept. Retrieved 5 May 2015.
 *
 */
Lattice Lattice::reduced_cell() const {
  ...
  while (k < 3) {
    for (int j = k - 1; j >= 0; j--) {
      double mu =
          reduced_lat.col(k).dot(ortho.col(j)) / ortho.col(j).squaredNorm();
      if (fabs(mu) > 0.5 + tol()) {
        reduced_lat.col(k) =
            reduced_lat.col(k) - round(mu) * reduced_lat.col(j);
    ...
    double mu2 = reduced_lat.col(k).dot(ortho.col(k - 1)) /
                 ortho.col(k - 1).squaredNorm();
    if ((ortho.col(k) + mu2 * ortho.col(k - 1)).squaredNorm() >
        (0.75) * ortho.col(k - 1).squaredNorm() + tol()) {
      k = k + 1;
    } else {
...
double Lattice::max_voronoi_measure(const Eigen::Vector3d &pos,
                                    Eigen::Vector3d &lattice_trans) const {
  Eigen::MatrixXd::Index maxv;
  double maxproj = (voronoi_table() * pos).maxCoeff(&maxv);

  lattice_trans = (2. * floor(maxproj / 2. + (0.5 - TOL / 2.)) /
                   m_voronoi_table.row(maxv).squaredNorm()) *
                  m_voronoi_table.row(maxv);
...
Eigen::Vector3i Lattice::enclose_sphere(double radius) const {
  ...
  // recip contains three column vectors of length 'radius' pointed along plane
  // normals
  auto lambda = [](double val) { return ceil(val); };
  return (inv_lat_column_mat() * recip)
      .cwiseAbs()
      .unaryExpr(lambda)
      .colwise()
      .maxCoeff()
...
Eigen::Vector3i Lattice::millers(Eigen::Vector3d plane_normal) const {
  ...
  // These are h, k, l
  // For miller indeces h, k and l
  // plane_normal[CART]=h*a.recip+k*b.recip+l*c.recip
  return scale_to_int(lat_column_mat().transpose() * plane_normal, tol());
}

/**
 *  Using miller indeces, the intercept of the plane is calculated. All
 * intercepts are multiplied by a factor such that their values are integers.
 * This corresponds to the plane being shifted so that it lands on three lattice
 * sites. Using these lattice sites new vectors A and B that lie on the desired
 * plane can be constructed. The choice of vector C is somewhat arbitrary.
 * lattice_in_plane will first construct C to be the shortest most orthogonal
 * vector possible. The user is then given the option to make C more orthogonal
 * to A and B in exchange for a greater length. The returned lattice will be the
 * one containing the most orthogonal C that is still smaller than max_vol
 */

Lattice Lattice::lattice_in_plane(Eigen::Vector3i millers, int max_vol) const {
  // John G 121030
  // Hold new lattice vectors in these. Then at the end we make an actual
  // Lattice out of it
  Eigen::Matrix3d surface_cell,
      last_surface_cell;  // Holds new lattice vectors, two of which are in the
                          // surface plane

  // Miller indeces of 100, 010 or 001 mean you don't need a new cell to expose
  // the plane, however you may want to reorient the vectors so that the ab
  // plane is exposed (useful for Structure::stitch)

  if (millers == Eigen::Vector3i(0, 1, 0)) {
    return Lattice(lat_column_mat().col(2), lat_column_mat().col(0),
                   lat_column_mat().col(1), tol());
  }

  else if (millers == Eigen::Vector3i(1, 0, 0)) {
    return Lattice(lat_column_mat().col(1), lat_column_mat().col(2),
                   lat_column_mat().col(0), tol());
  }
  ...
  // Miller indeces of xx0, 0xx or x0x mean one of the lattice vectors is the
  // same as one of the primitive lattice vectors. This means we only need to
  // get TWO points that are on the plane, which we connect to get the second
  // lattice vector
    ...
    // In actualility, the inverse miller of 0 is infinity, we set it to 0 here
    // so that we can use scale_to_int without going crazy. Since we won't be
    // using this inverse miller index it's OK
    ...
    H_miller_point =
        inv_miller[(zero + 1) % 3] * lat_column_mat().col((zero + 1) % 3);
    K_miller_point =
        inv_miller[(zero + 2) % 3] * lat_column_mat().col((zero + 2) % 3);
    ...
    // We'll want to find points that lie on the plane AND land of lattice
    // points. In order to do this we need the miller inverses multiplied by a
    // factor that makes them integers
    ...
    // Get three vectors that connect the three points on the plane to each
    // other. Any two of the following vectors could be used for constructing
    // the new lattice, but it's convenient to pick the two most orthogonal
    // vectors
    ...
    // John G 121212
    // The vectors that we got at this point are valid, but sometimes larger
    ...
    // Try dividing by integers and see if they're still lattice combinations.
    // If they are shorten and continue
    for (int i = 0; i < 3; i++) {
      int maxval =
          round(templat.col(i).norm() / lat_column_mat().col(i).norm() + 1);
      for (int j = maxval; j > 1; j--) {
        Eigen::Vector3d shortened = inv_lat_column_mat() * templat.col(i) /
                                    j;  // Shorten and convert to fractional
        bool combo = true;
  ...
  //\John G 121212
  // We now have lattice vectors a and b. The c vector can be any vector that is
  // a linear combination of the original primitive cell lattice vectors.
  // Ideally the vector will be short and orthogonal to the plane.
  Eigen::Vector3d normal;
  Eigen::Vector3i L_combination;
  int factor;

  normal = inv_lat_column_mat() *
           surface_cell.col(0).cross(surface_cell.col(1));  // 101112
  factor = 1;

  // Divide by largest value in normal vector. We'll do something similar to
  // when finding the miller indeces. We'll approximate the real normal vector
  // with an integer normal vector, increasing the "resolution" of the integer
  // vector in steps, until the desired accuary is reached.
  if (fabs(normal[0]) >= fabs(normal[1]) &&
      fabs(normal[0]) >= fabs(normal[2]) && normal[0] != 0) {
    normal = normal / normal[0];
  }

  else if (fabs(normal[1]) >= fabs(normal[2]) &&
           fabs(normal[1]) >= fabs(normal[0]) && fabs(normal[1]) != 0) {
  ...
  // orthoscore represents how close the linear combination is to the plane
  // normal. 1 is perfect, 0 is stupid.
  double orthoscore = 1;
  ...
  do {
    ...
    // After getting the normal vector in terms of integers, make a linear
    // combination of the initial lattice vectors to get third new lattice
    // vector
    surface_cell.col(2) = lat_column_mat() * L_combination.cast<double>();
    orthoscore =
        fabs(cos(CASM::angle(lat_column_mat() * normal, surface_cell.col(2))));
    // Only use new linear combination if it's more orthogonal than the previous
    // one
    if (orthoscore > torthoscore + TOL) {
      torthoscore = orthoscore;

      last_surface_cell =
          surface_cell;  // Remember currect generated cell, in case we can't
                         // find anything better later
      one_found = true;

      new_vol = fabs(surface_cell.col(2).dot(
          surface_cell.col(0).cross(surface_cell.col(1))));
    }
    ...
    if (factor == 100) {
      ...
      std::cerr << "Reached an outrageous size. Returning last generated cell"
                << std::endl;
      surface_cell = last_surface_cell;
      break;
    }

  } while (new_vol / this->volume() < max_vol &&
           orthoscore < 1);  // John G 121030

  Lattice surface_lat(surface_cell.col(0), surface_cell.col(1),
                      surface_cell.col(2));
...
/// - Then compares via standard_orientation_compare(this->lat_column_mat(),
/// RHS.lat_column_mat(), TOL)
bool Lattice::operator<(const Lattice &RHS) const {
  ...
  return standard_orientation_compare(lat_column_mat(), RHS.lat_column_mat(),
                                      TOL);
}
...
//*******************************************************************************************
//
//  Finds "superduper" Lattice L_{sd} (represented as a matrix with lattice
//  vectors as its columns such that L_{sd} satisfies
//        L_{sd} = L_1 * N_1 = L_2 * N_2,     (*1*)
//  where N_1 and N_2 are integer matrices such that Eq.(*1*) is satisfied and
//  det(N_1) and det(N_2) are minimized.
//
//  It is assumed that L_1 = L * M_1 and L_2 = L * M_2  (i.e., L_1 and L_2 are
//  superlattices of PRIM lattice L having integer transformation matrices M_1
//  and M_2, respectively).
//
//  Algorithm proceeds by noting inv(L_2)*L_1 = N_2*inv(N_1) = inv(M_2)*inv(M_1)
//  = A/n, where A is an integer matrix and n = det(M_2). Assuming that 'n' is
//  small (n<10000), we can attempt to find 'n' and 'A'.
//
//  Solution: N_2 = A*N_1/n, s.t. det(N_1) is minimized and N_2 is integer
//
//  To minimize det(N_1), find smith normal form A = U*S*V, where U and V have
//  det(1), S is diagonal, and all entries are integer.  Then choose N_1 =
//  inv(V)*R, where R is a diagonal integer matrix with entries
//             R(i,i)=n/gcf(S(i,i),n)
...
std::pair<bool, Eigen::Matrix3d> is_superlattice(const Lattice &scel,
  ...
  Eigen::Matrix3d T_rounded = lround(T_exact).cast<double>();
  Eigen::Matrix3d diff =
      unit.lat_column_mat() - scel.lat_column_mat() * T_rounded.inverse();
  Eigen::Vector3d error = (diff.transpose() * diff).diagonal();

  bool result = almost_zero(error, 2. * tol) && !almost_zero(T_rounded, tol);
...
Eigen::Matrix3l make_transformation_matrix_to_super(const Lattice &tiling_unit,
  ...
  // TODO: convention is usually "prim" always goes first, but this is
  // contradicte by is_superlattice. Which should change?
  std::tie(is_integer_transformation, transformation_matrix) =
      is_superlattice(superlattice, tiling_unit, tol);

  if (!is_integer_transformation) {
    throw std::runtime_error(
        "The provided tiling unit and superlattice are not related by a "
        "non-singular integer transformation.");
  }
...
Eigen::Vector3d robust_pbc_displacement_cart(xtal::Lattice const &lattice,
                                             Eigen::Vector3d const &r1,
                                             Eigen::Vector3d const &r2) {
  Eigen::Vector3d disp_cart = fast_pbc_displacement_cart(lattice, r1, r2);
