  ...
  void _from_json(const BasicStructure &struc, const jsonParser &json) const {
    std::cerr
        << "WARNING: Attempting to populate a const Structure/BasicStructure "
           "from a JSON object.  This is not allowed.\n";
  }
...
jsonParser &SimpleJSonSiteStructure<IsConst>::to_json(jsonParser &json) const {
  ...
  for (; it != end_it; ++it) {
    ...
    for (; it2 != end_it2; ++it2) json[m_prefix + "basis"].push_back(*it2);
  }
  return json;
}

template <bool IsConst>
void SimpleJSonSiteStructure<IsConst>::_from_json(
    BasicStructure &struc, const jsonParser &json) const {
  ...
  try {
    ...
    if (tstr == "direct" || tstr == "Direct") mode = FRAC;
    ...
    throw std::runtime_error(
        std::string("Unable to parse Structure/BasicStructure from JSON "
                    "object.  One or more tags were improperly specified:\n") +
        ex.what());
  }
  return;
}
...
template <bool IsConst>
void from_json(const SimpleJSonSiteStructure<IsConst> &jstruc,
               const jsonParser &json) {
  jstruc.from_json(json);
}

template <bool IsConst>
jsonParser &to_json(const SimpleJSonSiteStructure<IsConst> &jstruc,
                    jsonParser &json) {
