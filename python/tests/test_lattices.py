...
def test_from_lattice_parameters():
    ...
    beta = 120
    gamma = 130
    lat = lattices.from_lattice_parameters(a=a,
                                           b=b,
                                           c=c,
                                           alpha=alpha,
                                           beta=beta,
    ...
    assert math.isclose(degrees(np.arccos(np.dot(L[:, 0], L[:, 1]) / (a * b))),
                        gamma)
    assert math.isclose(degrees(np.arccos(np.dot(L[:, 0], L[:, 2]) / (a * c))),
                        beta)
    assert math.isclose(degrees(np.arccos(np.dot(L[:, 1], L[:, 2]) / (b * c))),
                        alpha)
...
def test_construct_all_lattices():
    ...
    assert isinstance(lattices.monoclinic(a=1.0, b=1.3, c=1.6, beta=96.0),
                      Lattice)
    assert isinstance(
        lattices.triclinic(a=1.0,
                           b=1.3,
                           c=1.6,
                           alpha=78.0,
                           beta=96.0,
